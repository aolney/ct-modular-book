...
For this reason, time-based effects for short periods of time are often described in terms of phase, and longer-period effects are described in terms of time.
...
Common parameters of delays include the length of the delay and the number of times it repeats.
...
Try patching up a delay into a single voice keyboard patch using the button in Figure \@ref(fig:twelvekey-env-delay).
...
This reverb module uses a combination of sliders and knobs for parameter settings, but these can again be both manually set and modulated to create interesting sounds.
...
<!-- MODAL HTML BLOCK -->
<!-- Include image of phaser fx in the div rather than bitcrusher? -->
```{r echo=F, out.width="100%"}
