...
<li>Stop, reset, and run the sequencers to make sure they are in sync</li>
<li>Move the VCO down to the right of the slide gate sectin</li>
...
```{r accent-sweep, echo=F, out.width="100%", fig.cap="(ref:accent-sweep)"}
...
Modifying envelopes when accents are received can be accomplished similarly to how we modified gates, i.e. by mixing the accent voltages with the regular MEG/VEG envelopes.
...
<li>On the row above the sequencer row, add Slew Limiter, QuadVCA/Mixer, Offset, Scope for the MEG and QuadVCA/Mixer and Scope for the VEG</li>
<li>Connect the accent sequencer (the order is PRASE) to Slew in and Slew out to MEG ADSR Decay. This will control decay on accented notes</li>
<li>Connect Slew out to VEG QuadVCA/Mixer input 1. This will send extra volume voltage on accented notes</li>
<li>Connect the Slew out to the MEG QuadVCA/Mixer input 1 and the MEG ADSR out to the same QuadVCA/Mixer input 2</li>
<li>Connect the MEG QuadVCA/Mixer mix out to the Offset in
