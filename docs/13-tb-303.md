...
The problem is to make a lead by recreating some of the fundamental characteristics of the Roland TB-303, an iconic instrument in acid house and subsequent genres of electronic music.
...
Recall the problem solving stages elaborated for sound design in Chapter \@ref(designing-a-kick-drum):
...
These problem solving stages mostly apply to emulating a 303 but with some slight variation because the current goal is not to create a specific sound.
Instead, the current goal is to recreate all sounds produced by a particular instrument.

For the understanding the problem stage, we need to know what the defining characteristics of the 303 are and what makes it special.
According to Robin Whittle, developer of the Devil Fish 303 mod, the most important aspects of the 303 sound are its sequencer, filter, filter envelope, and some minor nuance to the oscillator.^[Most information is on Robin's [main website](https://www.firstpr.com.au/rwi/dfish/index.html) including the Devil Fish manual. Additional information is on the [Analogue Heaven mailing list](http://www.retrosynth.com/ah/).]
...
Also in the implement a plan stage, we will address the subproblems of sequencing, oscillators, filter, and envelopes using the some overall four problem solving stages.
