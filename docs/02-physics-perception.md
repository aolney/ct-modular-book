...
Unfortunately, the decibel is a bit harder to understand that Hz, and there are many variations of it used for different purposes.
...
If we call the reference sound $S_0$ and the sound we are measuring $S$, then the loudness of $S$ is $10 * log_{10} * (S/S_0)$ dB.
Under this definition of dB, a 3 dB increase in loudness means amplitude has doubled and an increase of 10 dB means that amplitude is 10 times greater.
