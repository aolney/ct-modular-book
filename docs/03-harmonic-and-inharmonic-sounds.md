...
In a circular membrane (a drum) or a circular plate (a cymbal) the nodes are lines and even circles, because the waves can now travel in two dimensions and reflect off the edges of the vibrating surface.
Some modes of a vibrating drum are shown in Figure \@ref(fig:drum-modes).
Each of these modes has a nodal circle around the perimeter of the drum head and  0, 1, or 2 nodal lines across the diameter of the drum head.
...
<div class="figure">
<iframe src="https://olney.ai/ct-modular-book/images/drum-modes.html" width="100%" height="130px" data-external="1"></iframe>
...
In general, the modes have nonharmonic frequency relationships to each other as shown in Table \@ref(tab:mode-freq).
...
The definition of harmonics is integer multiples of the fundamental, and the frequency relationships in Table \@ref(tab:mode-freq) are clearly not integers.
...
There are several prominent types of noise with different acoustic properties.
The noise types have been given [color names](https://en.wikipedia.org/wiki/Colors_of_noise) that may be helpful for remembering which is which, though in some cases the color names seem a bit arbitrary.
The first and most commonly though of noise is white noise.
This is the kind of noise that you see on an old television set (the "snow" on channel with no broadcast).
White noise has equal energy across its spectrum.
...
All other colors of noise are based on white noise but change the distribution of energy in the frequency spectrum in some way, i.e. increase the amplitude of some frequencies and decrease the amplitude of others, as shown in Figure

**TODO** insert figure with noise color graphs/sounds from https://en.wikipedia.org/wiki/Colors_of_noise

Pink noise balances energy across each octave to roughly approximate human perception.
To do this energy (amplitude) is reduced as frequency increases.
Brown noise (also called red noise) is like pink noise but reduces energy more quickly as frequency increases.
...
One might consider grey noise to the the evolution of pink noise.
